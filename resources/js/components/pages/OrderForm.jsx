...
const OrderForm = ({
    ...
    useEffect(() => {
        ...
        if (!visible) {
            ...
            setStep(2);
        } else if (fields.people && fields.date && fields.skip) {
            setStep(1);
        }
    }, [visible]);
    ...
    function submitForm() {
        ...
        axios
            .post(`${window.location.origin}/api/reservation`, {
                ...data,
                nParticipants: fields.people,
                experience_id: fields.experience.id,
                date: moment(fields.date).format("YYYY-MM-DD"),
                partner_id: currentPartner?.id ? currentPartner.id : null,
            })
            .then((response) => {
    ...
    return (
        ...
        >
            ...
            >
                <Content>{stepItems[step]}</Content>
            </Form>
        </ModalContainer>
    );
};
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        fetchPartnerFromUrl: (filters) =>
            dispatch(fetchPartnerFromUrl(filters)),
